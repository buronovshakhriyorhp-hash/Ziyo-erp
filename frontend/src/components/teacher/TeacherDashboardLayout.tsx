...
import { Bell, Menu, Search, UserCircle, LogOut, Settings, ShieldAlert } from 'lucide-react';
...
export default function TeacherDashboardLayout() {
    ...
    const getPageTitle = () => {
        if (location.pathname === '/teacher-portal') return 'Asosiy Oyna';
        if (location.pathname.startsWith('/teacher-portal/groups')) return 'Guruhlarim';
        if (location.pathname.startsWith('/teacher-portal/attendance')) return 'Davomat';
        if (location.pathname.startsWith('/teacher-portal/homework')) return 'Uy vazifalari';
        if (location.pathname.startsWith('/teacher-portal/settings')) return 'Sozlamalar';
        return 'O\'qituvchi Portali';
    };
    ...
    return (
        <div className="flex bg-[#0f172a] min-h-screen font-sans text-slate-300 overflow-hidden font-inter selection:bg-emerald-500/30">
            ...
            <TeacherDashboardSidebar isMobileOpen={isMobileMenuOpen} onMobileClose={() => setIsMobileMenuOpen(false)} />
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="text-right hidden sm:block">
                                    ...
                                    <p className="text-[10px] font-medium text-emerald-400 uppercase tracking-widest mt-1">Ustoz</p>
                                </div>
                            ...
                            {isProfileDropdownOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-48 bg-[#1e293b] rounded-2xl shadow-2xl border border-white/10 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                                        ...
                                        <div className="p-2">
                                            {(user?.roleName === 'admin' || user?.roleName === 'manager' || (user as any)?.role === 'admin') && (
                                                <button
                                                    onClick={() => {
                                                        setIsProfileDropdownOpen(false);
                                                        navigate('/dashboard');
                                                    }}
                                                    className="w-full text-left px-3 py-2 text-sm text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 rounded-xl transition-colors flex items-center gap-2 mb-1"
                                                >
                                                    <ShieldAlert className="w-4 h-4" /> Admin Boshqaruv
                                                </button>
                                            )}
                                            <button
                                                onClick={() => {
                                                    setIsProfileDropdownOpen(false);
                                                    navigate('/teacher-portal/settings');
                                                }}
